...
exports.insertData = ( value ) => {
  let _sql = "insert into users set name=?,email=?,pass=?,avator=?,moment=?,token=?,token_exptime=?,status=?;"
  console.log(_sql);
...
exports.insertComment = ( value ) => {
  let _sql = "insert into comment set name=?,content=?,md=?,moment=?,postid=?,avator=?;"
  console.log(_sql);
  return query( _sql, value )
}
// 通过名字查找用户
...
exports.findUserByPage = ( page ) => {
  let _sql = ` select * from users order by id asc limit ${(page-1)*10},10;`
  console.log(_sql);
  return query( _sql)
}
...
exports.findPostBykeywordsPage = (searchType,keywords,page) => {
  let _sql = ``;
  if(searchType==='article') {
    _sql = `select * from posts where content like '%${keywords}%' order by id desc limit ${(page-1)*10},10 ;`
  } else{
    _sql = ` select * from posts where name="${keywords}" order by id desc limit ${(page-1)*10},10 ;`
  }
  console.log(_sql);
  return query( _sql)
}
