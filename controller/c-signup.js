          ...
          .then(res => {
            ...
            if (res.affectedRows == 1) {
              ...
              var mail = {
                ...
                // 邮件内容，HTML格式
                html:
                  "亲爱的<b style='color:red'>" +
                  name +
                  "</b>用户!<br/>感谢您注册该论坛系统。<br/>为了更好的使用,请点击下面的链接激活您的帐号。<br/>" +
                  "<a href='http://" +
                  ctx.host +
                  "/active?verify=" +
                  token +
...
exports.getUserlist = async ctx => {
  await userModel.findUserByPage(1).then(result => {
    res = result;
  });
  await userModel.findAllUserCount().then(result => {
